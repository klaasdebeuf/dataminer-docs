...
1. Create an outbound rule:
   ...
   > [!NOTE]  
   > Firewall rules can optionally be restricted to the **specific database server IP** for a more secure setup, ensuring that only the intended database host can communicate over these ports.
...
1. For a MSSQL database only, on every DMA in the DMS, share the *C:\Skyline DataMiner\System Cache\Offload* folder:
   ...
   >  
   > **Alternatively**, in domain environments, granting read/write permissions to the **DMA’s computer account** also works and is more secure, since the domain computer account can authenticate automatically when accessing the shared folder.
...
> - To troubleshoot offload database issues,you can check the log file at C:\Skyline DataMiner\Logging\SLDBConnection.txt.
